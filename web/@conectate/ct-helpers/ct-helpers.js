 ...
 */
export function browserCapabilities(userAgent) {
    /**
     * Return whether `version` is at least as high as `atLeast`.
     */
    function versionAtLeast(atLeast, browserVersion) {
    ...
    function since(...atLeast) {
        return (ua) => versionAtLeast(atLeast, parseVersion(ua.browserVersion));
    }
    /**
     * Make a predicate that checks if the OS version is at least this high.
     */
    function sinceOS(...atLeast) {
        return (ua) => versionAtLeast(atLeast, parseVersion(ua.osVersion));
    }
    const chrome = {
        es2015: since(49),
        es2016: since(58),
    ...
    const browserPredicates = {
        ...
        safari: {
            ...
            push: (ua) => {
                return (versionAtLeast([9], parseVersion(ua.browserVersion)) &&
 ...
 * Sleep Promise
 */
export let sleep = (time) => new Promise(resolve => setTimeout(() => resolve(0), time));
/**
 * Generate Random ID
 */
export class PushID {
    constructor() {
        ...
        this.PUSH_CHARS = '-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz';
